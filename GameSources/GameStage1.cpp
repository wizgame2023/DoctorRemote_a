...
//ビューとライトの作成
	void GameStage1::CreateViewLight() {
	...
	void GameStage1::CreateEffect()//エフェクト生成
	{
	...
	//Playerを追加する関数
	void GameStage1::CreatePlayer()//改善すべき点
	{
		//Playerの出現場所を決める
		float deg = -180;
	...
	//敵を作成
	void GameStage1::CreateEnemy()
	{
		auto ptrEnemy = AddGameObject<Enemy>(Vec3(-26.0f, 0.5f, -11.5f), Vec3(-0.0f, 0.0f, 0.0f), Vec3(3.0f, 3.0f, 3.0f));
		SetSharedGameObject(L"Enemy", ptrEnemy);//ゲームオブジェクトを取得

		auto EnemyPos = ptrEnemy->GetComponent<Transform>()->GetPosition();
		EnemyPos.y = 0.5;
		//Enemyのムービーシーン
		AddGameObject<EnemyMovieManager>(EnemyPos+Vec3(-10.0f,0.0f,0.0f), Vec3(30.0f, 0.1f, 40.0f), Vec3(-25.45f, 4.0f, -20.8f), EnemyPos);

	}

	//敵の欠片を作成
	void GameStage1::CreateEnemyPiece() {

		vector<vector<Vec3>> vec = {
			{//1			
				Vec3(5.0f,0.1f,-28.0f),
				Vec3(0.0f,0.0f,0.0f),
				Vec3(2.0f,2.0f,2.0f)

			},
			{//2
				Vec3(-6.0f,0.1f,-33.0f),
				Vec3(0.0f,0.0f,0.0f),
				Vec3(2.0f,2.0f,2.0f)

			},
			{//3
				Vec3(-3.0f,0.1f,-20.0f),
				Vec3(0.0f,0.0f,0.0f),
				Vec3(2.0f,2.0f,2.0f)

			},
			{//4
				Vec3(5.0f,0.1f,32.0f),
				Vec3(0.0f,0.0f,0.0f),
				Vec3(2.0f,2.0f,2.0f)
			},
			{//5
				Vec3(-4.0f,0.1f,28.0f),
				Vec3(0.0f,0.0f,0.0f),
				Vec3(2.0f,2.0f,2.0f)
	...
	void GameStage1::RandamPiecePosition(Vec3 originPosition)//引数を中心にランダムにかけらが置かれる
	{
		int randamCount = 0;
	...
	void GameStage1::CreateEnemyPiece2()
	{
	...
	void GameStage1::CerateBreakEnemyPiece()//壊れる壁の先にあるかけら
	{

		Vec3 Pos[] = { Vec3(0.0f,0.3f,45.0f),Vec3(-45.0f,0.3f,-45.0f),Vec3(-45.0f,0.3f,45.0f),Vec3(45.0f,0.3f,45.0f),Vec3(45.0f,0.3f,-45.0f),Vec3(0.0f,0.3f,-45.0f) };//BigPieceのPosision一覧
	...
	void GameStage1::CreateMiniMap()
	{
		...
		auto miniMap = AddGameObject<Sprite>(Lenght, Lenght, L"MiniMapStage1", Vec3(640.0f - (Lenght / 2.0f) - 50.0f, 400.0f - (Lenght / 2.0f) - 50.0f, 0.0f), 5);//ミニマップ生成
	...
	void GameStage1::CollisionActive(bool On)
	{
	...
	void GameStage1::SetCollisionSwich(bool ONorOFF)
	{
	}


	void GameStage1::OnCreate() {
		try {
			...
			AddGameObject<RandCreateManager>(L"kakeraMapLevel1.csv", 100, 30);//ランダムにかけらが出るようになる
