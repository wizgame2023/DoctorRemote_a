	...
	{	
		...
		if (m_dashLight) {
			if (player->GetDashFlag()) {
				if (m_dashLight->GetOnOff() == true)return;//透明にしたいときは赤にならないようにする
		...
		if (scene->GetTime() < 60) {
			if (m_clock->GetOnOff() == true)return;//透明にしたいときは赤にならないようにする
  			m_clock->SetColor(Col4(1.0f, 0.0f, 0.0f, 1.0f));
		}
		else {
			if (m_clock->GetOnOff() == true)return;//透明にしたいときは緑にならないようにする
