...
//ビューとライトの作成
	...
	void TutorialStage::CreatePlayer()//改善すべき点
	{
		...
		shared_ptr<Player> ptrPlayer = AddGameObject<Player>(Vec3(-27.5f, 0.5f, -13.0f), Vec3(0.0f, rad, 0.0f),200.0f);//Player生成
	...
	void TutorialStage::CreateMiniMap()
	{
		...
		for (int i = 0; i < m_BigPieceLength; i++)
		{
			...
			auto miniMapBigPiece = AddGameObject<MiniMapBigPiece>(6.0f, 6.0f, L"MiniMapBigPiece", Vec3(StartPos.x + (PiecePos.x * Bairitu), StartPos.y + (PiecePos.z * Bairitu), 0.0f), 6, BigPieceName);
	...
	{		
		...
		if (ptrPlayer->GetRadarFlag() && m_CareerFlag == 0)
		{
			//敵を生成
			CreateEnemy();
			//GetSharedGameObject<StageCollisionManager>(L"StageCollisionManager")->SetCollisionSwhich(false);//デバック用コリジョンをオフにする
