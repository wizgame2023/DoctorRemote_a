...
//ビューとライトの作成
	void GameStage11::CreateViewLight() {
	...
	void GameStage11::CreateEffect()//エフェクト生成
	{
	...
	//Playerを追加する関数
	void GameStage11::CreatePlayer()//改善すべき点
	{
		//Playerの出現場所を決める
		float deg = 90;
	...
	//敵の欠片を作成
	void GameStage11::CreateEnemyPiece() {
		...
		vector<vector<Vec3>> vec = {
			...
			{//4
				Vec3(42.3f,0.1f,-7.0f),
				Vec3(0.0f,0.0f,0.0f),
				Vec3(2.0f,2.0f,2.0f)
			},
			{//5
				Vec3(47.0f,0.1f,-15.9f),
				Vec3(0.0f,0.0f,0.0f),
				Vec3(2.0f,2.0f,2.0f)

			},
			{//6
				Vec3(-15.0f,0.1f,0.5f),
				Vec3(0.0f,0.0f,0.0f),
				Vec3(2.0f,2.0f,2.0f)
			},
			{//7
				Vec3(-33.0f,0.1f,16.0f),
				Vec3(0.0f,0.0f,0.0f),
				Vec3(2.0f,2.0f,2.0f)

			},
			{//8
				Vec3(-49.0f,0.1f,9.4f),
				Vec3(0.0f,0.0f,0.0f),
				Vec3(2.0f,2.0f,2.0f)

			},
			{//9
				Vec3(-54.0f,0.1f,-5.0f),
				Vec3(0.0f,0.0f,0.0f),
				Vec3(2.0f,2.0f,2.0f)

			},
			{//10
				Vec3(-57.0f,0.1f,22.0f),
				Vec3(0.0f,0.0f,0.0f),
				Vec3(2.0f,2.0f,2.0f)

			},
			{//11
				Vec3(54.0f,0.1f,-4.6f),
				Vec3(0.0f,0.0f,0.0f),
				Vec3(2.0f,2.0f,2.0f)

			},
			{//12
				Vec3(-10.0f,0.1f,-42.0f),
				Vec3(0.0f,0.0f,0.0f),
				Vec3(2.0f,2.0f,2.0f)

			},
			{//12
				Vec3(-21.0f,0.1f,-62.0f),
				Vec3(0.0f,0.0f,0.0f),
				Vec3(2.0f,2.0f,2.0f)

			},
			{//13
				Vec3(-29.5f,0.1f,-51.8f),
				Vec3(0.0f,0.0f,0.0f),
				Vec3(2.0f,2.0f,2.0f)
	...
	void GameStage11::RandamPiecePosition(Vec3 originPosition)//引数を中心にランダムにかけらが置かれる
	{
		int randamCount = 0;
	...
	void GameStage11::CreateEnemyPiece2()
	{
	...
	void GameStage11::CerateBreakEnemyPiece()//壊れる壁の先にあるかけら
	{
		//BigPieceのPos一覧
		Vec3 Pos[] =
		{
		...
		for (int r = 0; r < stageMap.size(); r++)
		{
			for (int c = 0; c < stageMap[0].size(); c++)
			{
				...
				case 11:
					...
					break;
				case 14:
					AddGameObject<Wall>(startPos + pos, Vec3(0, 0, 0), Vec3(4.0, 10, 1.0));
					break;
				case 15:
	...
	void GameStage11::CreateMiniMap()
	{
		...
		auto miniMap = AddGameObject<Sprite>(Lenght, Lenght, L"MiniMapStage10", StartPos, 5);//ミニマップ生成
	...
	void GameStage11::SetCollisionSwich(bool ONorOFF)
	{
	}


	void GameStage11::OnCreate() {
		try {
			auto scene = App::GetApp()->GetScene<Scene>();
			scene->SetGameStage(10);
			...
			AddGameObject<RandCreateManager>(L"kakeraMapLevel10.csv", 150, 70);//ランダムにかけらが出るようになる
			...
			auto joinManager = AddGameObject<JoinManager>(Vec3(0.0f, 0.5f, 0.0f), Vec3(13.0f, 0.5f, 0.0f));//ステージ開始時の演出
