...
//ビューとライトの作成
	...
	void GameStage9::CreateEffect()//エフェクト生成
	{
	...
	//Playerを追加する関数
	void GameStage9::CreatePlayer()//改善すべき点
	{
		//Playerの出現場所を決める
		float deg = -180;
	...
	void GameStage9::CreateEnemy()
	{
		...
		{
		case 0:
			ptrEnemy = AddGameObject<Enemy>(Vec3(-60.0f, 0.5f, 13.0f), Vec3(-0.0f, 0.0f, 0.0f), Vec3(0.8f, 0.8f, 0.8f));
			break;
		case 1:
	...
	//敵の欠片を作成
	void GameStage9::CreateEnemyPiece() {
	...
	void GameStage9::RandamPiecePosition(Vec3 originPosition)//引数を中心にランダムにかけらが置かれる
	{
		int randamCount = 0;
	...
	void GameStage9::CreateEnemyPiece2()
	{
	...
	void GameStage9::CreateMiniMap()
	{
		float Lenght = 225.0f;//ミニマップの直径
		auto miniMap = AddGameObject<Sprite>(Lenght, Lenght, L"MiniMapStage9", Vec3(640.0f - (Lenght / 2.0f) - 50.0f, 400.0f - (Lenght / 2.0f) - 50.0f, 0.0f), 5);//ミニマップ生成
	...
	void GameStage9::SetCollisionSwich(bool ONorOFF)
	{
	}
	...
	void GameStage9::OnCreate() {
		try {
			...
			AddGameObject<RandCreateManager>(L"kakeraMapLevel9.csv", 150, 30);//ランダムにかけらが出るようになる
