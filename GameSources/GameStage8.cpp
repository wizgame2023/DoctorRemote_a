...
//ビューとライトの作成
	...
	void GameStage8::CreatePlayer()//改善すべき点
	{
		...
		shared_ptr<Player> ptrPlayer = AddGameObject<Player>(Vec3(0.0f, 0.5f, 0.0f), Vec3(0.0f, rad, 1.7f),1.7f);
	...
	void GameStage8::CreateMiniMap()
	{
		...
		auto miniMap = AddGameObject<Sprite>(Lenght, Lenght, L"MiniMapStage8", Vec3(640.0f - (Lenght / 2.0f) - 50.0f, 400.0f - (Lenght / 2.0f) - 50.0f, 0.0f), 5);//ミニマップ生成
	...
	void GameStage8::OnCreate() {
		try {
			...
			//敵のかけらを表示
			CreateEnemyPiece();
			AddGameObject<RandCreateManager>(L"kakeraMapLevel8.csv", 150, 45);//ランダムにかけらが出るようになる
